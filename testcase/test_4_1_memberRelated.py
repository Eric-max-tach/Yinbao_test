    ...
    def testRechargeAndCouponIssue(self):
        ...
        """
            ...
            判断逻辑：
                ...
                9、使用“is_added”标志位，初始值为False，用于标志是否检测到新添加的优惠券
                10、获取显示在当前视图中的优惠券元素列表，并得到其优惠券号码。检查该号码是否已经记录在字典中，如果不存在则说明已经找到新添加的优惠券，将“is_added”置为True，将该号码记录在字典中，同时跳出循环
                ...
                15、通过断言判断“is_added”是否为True，如果为True则说明优惠券添加成功，否则添加失败
        """
        ...
        while True:
            ...
            for i in range(len(coupon_no_eles)):
                coupon_no = coupon_no_eles[i].text
                if coupon_no not in coupon_number_dic:
                    coupon_number_dic[coupon_no] = 1  # 如果该优惠券不在列表中，则说明优惠券添加成功
                    is_added = True
                    break
            if last_coupon_number_index == coupon_no_eles[-1].text: # 用于判断是否滑动到列表底部
                break
        ...
        # 判断优惠券是否被成功添加，如果“is_added”标志位为True，这说明优惠券添加成功
